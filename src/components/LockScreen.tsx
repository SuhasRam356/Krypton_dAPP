...
function getErrorMessage(error: unknown): string {
  return error instanceof Error ? error.message : 'Unknown vault error';
}
...
export default function LockScreen({ children }: { children: ReactNode }) {
  ...
  // Manual and idle lock. Reloading clears sensitive Zustand state from memory.
  useEffect(() => {
    ...
    window.addEventListener('krypton-lock', lock);
    ...
    return () => {
      if (timer) window.clearTimeout(timer);
      window.removeEventListener('krypton-lock', lock);
    ...
    <div className="flex h-screen w-full items-center justify-center bg-[#0d1117] text-white">
      <div className="glass-panel p-8 rounded-2xl w-full max-w-sm text-center shadow-2xl border border-[rgba(88,166,255,0.1)]">
        ...
        <p className="text-sm text-gray-400 mb-6">
          {isNewDevice
            ? 'Set a PIN to encrypt your keys and messages on this device.'
            : 'Enter your PIN to decrypt your vault.'}
        </p>
