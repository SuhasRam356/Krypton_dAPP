...
function StatCard({
  ...
  return (
    <div className="glass-panel rounded-xl p-5 flex items-center space-x-4 shadow-lg">
      ...
      <div>
        <p className="text-[11px] uppercase tracking-widest text-gray-500 font-bold mb-0.5">
          {label}
        </p>
...
export default function Dashboard() {
  const { messages, contacts, keys, walletState, selfDestructTTL, isRelayConnected, offlineQueue } =
    useKryptonStore();
  const [peerEvents, setPeerEvents] = useState<PeerEvent[]>([]);
  const [activePeers, setActivePeers] = useState<Map<string, 'connected' | 'disconnected'>>(
    new Map()
  );
  const [fallbackIdentityCreatedAt] = useState(() => Date.now());
  const [now, setNow] = useState(() => Date.now());
  const identityCreatedAt = keys?.createdAt ?? fallbackIdentityCreatedAt;
  ...
  // In-chat transfers
  const transfers = useMemo(() => messages.filter((m) => m.isCryptoTransfer), [messages]);

  const connectedPeerCount = Array.from(activePeers.values()).filter(
    (s) => s === 'connected'
  ).length;
  const lastSync = stats.networkStats.lastSyncTimestamp
    ? `${Math.round((now - stats.networkStats.lastSyncTimestamp) / 1000)}s ago`
    : 'N/A';

  const identityAgeMs = now - identityCreatedAt;
  const identityAge =
    identityAgeMs < 60000
      ? `${Math.round(identityAgeMs / 1000)}s`
      : identityAgeMs < 3600000
        ? `${Math.round(identityAgeMs / 60000)}m`
    ...
    <div className="flex flex-col h-full w-full max-w-7xl mx-auto p-6 space-y-8 overflow-y-auto">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">
          Real-time P2P network, messaging analytics, portfolio overview, and security posture.
        </p>
      </div>
      {/* ═══════ Top stat cards ═══════ */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={stats.networkStats.messagesRelayed}
          color={COLORS.blue}
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
        ...
        <StatCard
          label="Contacts"
          value={contacts.filter((c) => !c.isAi).length}
          color={COLORS.purple}
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
          ...
          <div className="space-y-3">
            ...
            {getPeers().map((url) => {
              ...
              return (
                <div
                  key={url}
                  className="flex items-center space-x-3 bg-black/20 rounded-lg p-3 border border-gray-800"
                >
                  <span
                    className={`w-3 h-3 rounded-full ${status === 'connected' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-red-500 shadow-[0_0_8px_rgba(248,113,113,0.6)]'}`}
                  />
                  <span className="text-sm text-gray-300 font-mono truncate flex-1">{url}</span>
                  <span
                    className={`text-[10px] font-bold uppercase tracking-wider ${status === 'connected' ? 'text-green-400' : 'text-red-400'}`}
                  >
                    {status}
                  </span>
                </div>
      ...
      <div className="glass-panel rounded-2xl p-6 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-widest font-bold mb-3">
              Message Volume (Sent vs Received)
            </p>
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={
                    stats.messageDayBuckets.length > 0
                      ? stats.messageDayBuckets
                      : [{ date: 'No data', sent: 0, received: 0 }]
                  }
                >
                  <CartesianGrid strokeDasharray="3 3" stroke="#21262d" />
                ...
                {topContacts.map((tc, i) => {
                  ...
                  return (
                    <div key={tc.contactId} className="flex items-center space-x-3">
                      <span className="text-xs text-gray-600 w-4 text-right font-bold">
                        {i + 1}
                      </span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-800">
          ...
          <div className="text-center">
            <p className="text-2xl font-black text-yellow-400">
              {stats.avgCiphertextOverhead.toFixed(1)}x
            </p>
            <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">
              E2EE Overhead
            </p>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    stroke="none"
                  >
                    {(walletPieData.length > 0 ? walletPieData : [{ name: 'Empty', value: 1 }]).map(
                      (_, i) => (
            ...
            {transfers.length === 0 ? (
              ...
              <div className="overflow-y-auto max-h-56 space-y-2">
                {transfers.map((tx) => {
                  ...
                  const otherName =
                    contacts.find((c) => c.id === (isMe ? tx.recipient : tx.sender))?.name ||
                    'Unknown';
                  return (
                    <div
                      key={tx.id}
                      className="flex items-center justify-between bg-black/20 rounded-lg p-3 border border-gray-800"
                    >
                      <div className="flex items-center space-x-3">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center ${isMe ? 'bg-red-500/10 text-red-400' : 'bg-green-500/10 text-green-400'}`}
                        >
                          <svg
                            className="w-4 h-4"
                            fill="none"
                          ...
                          >
                            <path
                              ...
                              strokeLinejoin="round"
                              strokeWidth={2}
                              d={isMe ? 'M5 10l7-7m0 0l7 7m-7-7v18' : 'M19 14l-7 7m0 0l-7-7m7 7V3'}
                            />
                          </svg>
                        </div>
                        <div>
                      ...
                      <span className={`font-bold ${isMe ? 'text-red-400' : 'text-green-400'}`}>
                        {isMe ? '-' : '+'}
                        {tx.transferAmount} {tx.transferSymbol}
                      </span>
                    </div>
                  );
                })}
      ...
      <div className="glass-panel rounded-2xl p-6 shadow-lg">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-black/20 rounded-xl p-5 border border-gray-800 text-center">
            ...
            <p className="text-[10px] text-gray-600 mt-1">+ Demo symmetric ratchet</p>
          </div>
          <div className="bg-black/20 rounded-xl p-5 border border-gray-800 text-center">
            <p className="text-xs text-gray-500 uppercase tracking-widest font-bold mb-2">
              Identity Age
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
          <div className="bg-black/20 rounded-xl p-5 border border-gray-800 text-center">
            ...
            <p
              className={`text-lg font-black ${selfDestructTTL ? 'text-orange-400' : 'text-gray-600'}`}
            >
              {selfDestructTTL ? `${selfDestructTTL}s` : 'Off'}
            </p>
            <p className="text-[10px] text-gray-600 mt-1">
              {selfDestructTTL ? '🔥 Messages auto-delete' : 'Disabled'}
            </p>
          </div>
          <div className="bg-black/20 rounded-xl p-5 border border-gray-800 text-center">
            <p className="text-xs text-gray-500 uppercase tracking-widest font-bold mb-2">
            ...
            <p
              className={`text-lg font-black ${offlineQueue.length > 0 ? 'text-yellow-400' : 'text-green-400'}`}
            >
              {offlineQueue.length}
            </p>
            <p className="text-[10px] text-gray-600 mt-1">
              {offlineQueue.length > 0 ? 'Pending delivery' : 'All delivered'}
            </p>
          </div>
          <div className="bg-black/20 rounded-xl p-5 border border-gray-800 text-center">
            <p className="text-xs text-gray-500 uppercase tracking-widest font-bold mb-2">
            ...
            <p
              className={`text-lg font-black ${isRelayConnected ? 'text-green-400' : 'text-red-400'}`}
            >
              {isRelayConnected ? 'Online' : 'Offline'}
            </p>
            <p className="text-[10px] text-gray-600 mt-1">
              {isRelayConnected ? 'Connected to relay' : 'Queuing messages'}
            </p>
          </div>
        </div>
              ...
              {contacts.filter((c) => !c.isAi).length === 0 ? (
                ...
                contacts
                  ...
                  .map((contact) => (
                    ...
                    >
                      <div
                        className={`w-8 h-8 rounded-full bg-gradient-to-tr ${contact.avatarColor || 'from-blue-500 to-cyan-500'} flex items-center justify-center`}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-gray-600 font-mono truncate">
                          {contact.id.slice(0, 12)}...{contact.id.slice(-6)}
                        </p>
                      </div>
