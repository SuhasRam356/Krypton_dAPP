...
export default function WalletDashboard() {
  ...
  const totalBalanceUSD = `$${walletState.assets
    .reduce((acc, asset) => {
      let price = 0;
    ...
    try {
      const response = await fetch('/api/ai', {
        ...
        body: JSON.stringify({
          ...
          systemPrompt:
            'You are a professional cryptocurrency portfolio analyst. Provide concise, insightful analysis.',
        }),
      });
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-5xl mx-auto p-6 space-y-6 relative">
      ...
      <div className="glass-panel p-8 rounded-2xl flex flex-col items-center justify-center relative overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center justify-between w-full mb-8">
          ...
          <div className="flex gap-3">
            {connectedAddress ? (
              <button
                onClick={disconnect}
                className="bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 px-4 py-2 rounded-lg font-semibold transition-all"
              >
                Disconnect MetaMask
              </button>
            ) : (
              <button
                ...
                className="bg-orange-500/20 hover:bg-orange-500/30 text-orange-400 border border-orange-500/30 px-4 py-2 rounded-lg font-semibold transition-all flex items-center"
              >
                {isConnecting ? (
                  'Connecting...'
                ) : (
        ...
        <span className="text-gray-400 text-sm font-medium tracking-wider uppercase mb-2 z-10">
          Total Balance
        </span>
        <h1 className="text-5xl font-extrabold text-white tracking-tight mb-6 z-10">
          {totalBalanceUSD}
        </h1>

        {walletError && (
          <div className="bg-red-500/10 border border-red-500/30 text-red-400 px-4 py-3 rounded-lg text-sm mb-3 z-10 relative">
            {walletError}
          </div>
        )}
        ...
        <div className="flex space-x-4 z-10">
          <button
            ...
            className="bg-[#58a6ff] hover:bg-[#1f6feb] text-white px-8 py-3 rounded-full font-semibold shadow-lg shadow-[#58a6ff]/20 transition-all flex items-center"
          >
          ...
          <button
            ...
            className="bg-[rgba(48,54,61,0.8)] hover:bg-[rgba(48,54,61,1)] text-white border border-gray-600 px-8 py-3 rounded-full font-semibold transition-all flex items-center shadow-lg"
          >
            <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                strokeLinecap="round"
          ...
          <div>
            ...
            <div className="flex items-center">
              ...
              <p className="text-sm font-semibold text-gray-200">
                {connectedAddress ? networkName : walletState.network}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="glass-panel rounded-2xl overflow-hidden flex-1 flex flex-col shadow-lg">
        ...
        <div className="overflow-y-auto">
          {walletState.assets.map((asset) => (
            <div
              key={asset.symbol}
              className="p-4 border-b border-[rgba(48,54,61,0.5)] last:border-0 hover:bg-[rgba(255,255,255,0.02)] transition-colors flex items-center justify-between"
            >
              <div className="flex items-center">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass-panel w-full max-w-sm rounded-2xl p-6 relative">
            ...
            <div className="bg-white p-4 rounded-xl flex justify-center mb-6">
              <QRCodeSVG value={activeAddress} size={200} />
            </div>
            <div className="bg-gray-800/50 p-3 rounded-lg border border-gray-700 flex items-center justify-between">
              <p className="font-mono text-xs text-gray-300 break-all">{activeAddress}</p>
              <button
                onClick={() => {
                  navigator.clipboard.writeText(activeAddress);
                  alert('Copied!');
            ...
            <p className="text-center text-xs text-gray-500 mt-4">
              Send only ERC-20 tokens on {walletState.network} to this address.
            </p>
          </div>
        </div>
      )}
                ...
                {connectedAddress && networkName === 'Sepolia' && (
                  <div className="bg-purple-500/10 border border-purple-500/30 rounded-lg px-3 py-2 text-center">
                    <p className="text-[10px] text-purple-400 font-bold uppercase tracking-widest">
                      🔗 Real On-Chain Transfer (Sepolia)
                    </p>
                  </div>
                )}
                <div>
