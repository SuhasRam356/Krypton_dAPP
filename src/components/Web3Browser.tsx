      ...
      <div className="flex items-end bg-[#05070a] px-2 pt-2 h-12 overflow-x-auto hide-scrollbar">
        {tabs.map((tab) => (
          ...
          >
            {/* Favicon Placeholder */}
            {tab.isLoading ? (
              <svg className="animate-spin h-3 w-3 mr-2 text-[#58a6ff]" viewBox="0 0 24 24">
                <circle
              ...
              <div className="w-3 h-3 rounded bg-white/10 mr-2 flex items-center justify-center text-[8px] font-bold">
                W
              </div>
            )}
      ...
      <div className="bg-[#161b22] p-2 flex items-center space-x-3 border-b border-[rgba(88,166,255,0.1)] z-20">
        ...
        <div className="flex-1 relative flex items-center">
          <form
            ...
            className="w-full flex items-center bg-[#0d1117] border border-gray-700 rounded-full pl-2 pr-4 py-1.5 focus-within:border-[#58a6ff] transition-colors relative"
          >
                ...
                <div className="absolute top-full left-0 mt-3 w-72 bg-[#161b22] border border-[rgba(88,166,255,0.2)] rounded-xl shadow-2xl p-4 z-50">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-white flex items-center">
                      <svg
                        className={`w-5 h-5 mr-2 ${activeTab.shieldsUp ? 'text-[#f87171]' : 'text-gray-500'}`}
                        fill="currentColor"
                        viewBox="0 0 24 24"
                      >
                        <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
                    ...
                    >
                      <span
                        className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${activeTab.shieldsUp ? 'translate-x-5' : 'translate-x-1'}`}
                      />
                    </button>
                  </div>
                  <p className="text-xs text-gray-400 mb-4 border-b border-gray-700 pb-4">
                    {activeTab.shieldsUp
                      ? 'Krypton is blocking trackers and scripts on this site to protect your privacy.'
                      : 'You are unprotected on this site.'}
                  </p>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-300">Trackers Blocked</span>
                    <span className="font-bold text-white">
                      {activeTab.shieldsUp ? activeTab.trackersBlocked : 0}
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="w-full max-w-3xl flex flex-col items-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mb-12">
                <div className="bg-[#161b22] border border-gray-800 rounded-2xl p-6 flex flex-col items-center justify-center shadow-lg">
                  <span className="text-4xl font-black text-[#f87171] mb-2">
                    {globalStats.trackers.toLocaleString()}
                  </span>
                ...
                <div className="bg-[#161b22] border border-gray-800 rounded-2xl p-6 flex flex-col items-center justify-center shadow-lg">
                  <span className="text-4xl font-black text-[#60a5fa] mb-2">
                    {globalStats.bandwidthMb.toFixed(1)} <span className="text-lg">MB</span>
                  </span>
                  <span className="text-sm text-gray-400 uppercase tracking-widest font-bold">
                    Bandwidth Saved
                  </span>
                ...
                <div className="bg-[#161b22] border border-gray-800 rounded-2xl p-6 flex flex-col items-center justify-center shadow-lg">
                  <span className="text-4xl font-black text-[#34d399] mb-2">
                    {globalStats.timeSavedMins.toFixed(1)} <span className="text-lg">m</span>
                  </span>
                  <span className="text-sm text-gray-400 uppercase tracking-widest font-bold">
                    Time Saved
                  </span>
              ...
              <div className="w-full grid grid-cols-4 gap-4 max-w-xl">
                {['DEX', 'NFTs', 'Social', 'DAOs'].map((cat, i) => (
                  <button
                    key={i}
                    className="bg-[#161b22] hover:bg-[#21262d] border border-gray-800 hover:border-gray-700 rounded-xl p-4 flex flex-col items-center transition-colors"
                  >
            ...
            {activeTab.isLoading && (
              <div className="absolute inset-0 bg-[#0d1117]/50 backdrop-blur-sm flex items-center justify-center z-10 pointer-events-none">
                <div className="bg-[#161b22] border border-gray-700 p-4 rounded-xl shadow-2xl flex items-center">
                  <svg
                    className="animate-spin -ml-1 mr-3 h-5 w-5 text-[#58a6ff]"
