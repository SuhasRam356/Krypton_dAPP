...
export default function SettingsPage() {
  ...
  const [copied, setCopied] = useState<'id' | 'wallet' | null>(null);

  const linkedWallet = connectedAddress || walletState?.address || (keys ? keys.ethAddress : "Not Generated");
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">Manage your cryptographic footprint on the Krypton network</p>
      </div>
      ...
      <div className="glass-panel rounded-2xl overflow-hidden shadow-2xl p-8 relative">
        ...
        <h2 className="text-xl font-bold text-white mb-6 z-10 relative flex items-center">
          <svg className="w-6 h-6 mr-3 text-[#58a6ff]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>
          Your Krypton ID
        </h2>
        ...
        <div className="space-y-6 z-10 relative">
          ...
          <div className="bg-[#0d1117] border border-gray-700 rounded-xl p-5">
            <div className="flex justify-between items-center mb-2">
              <label className="text-xs text-gray-400 uppercase tracking-wider font-semibold">Linked Wallet (crypto transfers only)</label>
              {connectedAddress && (
            ...
            <p className="text-gray-500 text-sm mb-3">Used only to send/receive real crypto in chat. Switching this wallet never affects your Krypton ID or message history.</p>
