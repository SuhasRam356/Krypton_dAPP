...
export default function ContactsPage() {
  const { contacts, addContact, removeContact } = useKryptonStore();
  ...
  const handleAddContact = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !kryptonId) return;

    // Convert generic color names to hex codes for the avatar
      ...
      <div className="glass-panel rounded-2xl overflow-hidden shadow-2xl flex-1">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
          {contacts.map(contact => (
            <div key={contact.id} className="bg-[rgba(33,38,45,0.6)] border border-[rgba(48,54,61,0.5)] rounded-xl p-5 hover:bg-[rgba(48,54,61,0.4)] transition-colors relative group">
              <div className="flex items-start justify-between mb-4">
                <div className={`w-14 h-14 rounded-full bg-gradient-to-tr ${contact.avatarColor} flex items-center justify-center shadow-lg shadow-black/20`}>
                  {contact.isAi ? (
                    <span className="text-2xl">🤖</span>
                  ) : (
                    <span className="text-xl font-bold text-white">{contact.name.charAt(0).toUpperCase()}</span>
                  )}
                </div>
                {!contact.isAi && (
                  <button onClick={() => removeContact(contact.id)} className="text-gray-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity p-1">
              ...
              <div className="mt-5 flex space-x-2">
                <Link href={`/chat?contactId=${contact.id}`} className="flex-1 bg-[rgba(255,255,255,0.05)] hover:bg-[rgba(255,255,255,0.1)] text-white text-center py-2 rounded-lg text-sm font-semibold transition-colors border border-white/10">
                  Message
                </Link>
                {!contact.isAi && (
                  <button className="flex-1 bg-[rgba(255,255,255,0.05)] hover:bg-[rgba(255,255,255,0.1)] text-white py-2 rounded-lg text-sm font-semibold transition-colors border border-white/10">
                    Send Crypto
                  </button>
                )}
              </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass-panel w-full max-w-md rounded-2xl p-6 relative shadow-2xl">
            ...
            <form onSubmit={handleAddContact} className="space-y-4">
              <div>
                <label className="block text-xs text-gray-400 uppercase tracking-wider mb-2 font-semibold">Display Name</label>
                <input 
                  type="text" 
                  value={name}
